...
- **PWA:** `sw.js` (App-Shell + `vendor/`/`fonts/`/Firebase-SDK cache-first; RTDB/Auth nie gecacht; nur `ok`/opaque-Antworten werden abgelegt). `manifest.json`, `icon.svg`.
