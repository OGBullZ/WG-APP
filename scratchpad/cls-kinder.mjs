...
await p.addInitScript(([s, d]) => {
  window.__wgSeed = s; window.__src = []; localStorage.setItem('wg_data', JSON.stringify(Object.fromEntries(Object.entries(s).map(([k, v]) => [k, Array.isArray(v) ? v : Object.values(v)]))));
...
const kinder = () => p.evaluate(() => [...document.querySelector('[data-testid=heute-a]').children].map(e => (e.getAttribute('data-testid') || e.className || e.tagName) + ':' + Math.round(e.getBoundingClientRect().height)));
console.log('VOR', JSON.stringify(await kinder()));
await p.evaluate(() => window.__wg.fire());
await p.waitForTimeout(1200);
console.log('NACH', JSON.stringify(await kinder()));
